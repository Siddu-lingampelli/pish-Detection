...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  if (tab.url.startsWith('about:') || tab.url.startsWith('edge://') || tab.url.startsWith('moz-extension://')) return;
...
async function scanInBackground(url, tabId) {
  try {
    ...
    const threats = deriveThreats(data).slice(0, 20).map(t => String(t).slice(0, 200));
    ...
    if (riskScore >= 70) {
      try {
        ...
        // Page may not allow injection (e.g. web store, PDF viewer)
      }
    ...
    // silent — don't spam service worker logs
  }
}
...
function showWarningOverlay(riskScore, threats) {
  if (document.getElementById('phishing-shield-overlay')) return;
  ...
  const threatItems = Array.isArray(threats) && threats.length
    ? threats.map(t => `<li>${esc(t)}</li>`).join('')
    : '<li>Suspicious activity detected</li>';

  overlay.innerHTML = `
    <div style="max-width: 500px; background: #111; border: 2px solid #ef4444; border-radius: 12px; padding: 32px; text-align: center; color: white;">
      <div style="font-size: 64px; margin-bottom: 16px;">&#9888;&#65039;</div>
      <h1 style="font-size: 28px; margin-bottom: 12px; color: #ef4444;">Warning: High Risk Website</h1>
      <p style="font-size: 16px; color: #aaa; margin-bottom: 24px;">
        This website has a risk score of <strong style="color: #ef4444;">${esc(riskScore)}/100</strong>
      </p>
      <div style="background: #1a1a1a; padding: 16px; border-radius: 8px; margin-bottom: 24px; text-align: left;">
        <strong style="color: #ef4444;">Detected Threats:</strong>
        <ul style="margin: 12px 0 0 24px; color: #ccc;">${threatItems}</ul>
      </div>
      <div style="display: flex; gap: 12px;">
        <button id="phishing-proceed" style="flex: 1; padding: 12px 24px; background: #333; border: 1px solid #555; color: white; border-radius: 6px; font-size: 14px; cursor: pointer; font-weight: 600;">Proceed Anyway (Not Recommended)</button>
        <button id="phishing-goback" style="flex: 1; padding: 12px 24px; background: white; border: none; color: black; border-radius: 6px; font-size: 14px; cursor: pointer; font-weight: 600;">Go Back to Safety</button>
      </div>
    </div>
  `;
